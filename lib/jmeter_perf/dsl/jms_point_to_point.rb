module JmeterPerf
  class DSL
    def jms_point_to_point(params={}, &block)
      node = JmeterPerf::JMSPoint_to_Point.new(params)
      attach_node(node, &block)
    end
  end

  class JMSPoint_to_Point
    attr_accessor :doc
    include Helper

    def initialize(params={})
      testname = params.is_a?(Array) ? 'JMSPoint_to_Point' : (params[:name] || 'JMSPoint_to_Point')
